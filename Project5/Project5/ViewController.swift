...
class ViewController: UITableViewController {
        ...
        if isPossible(word: lowerAnswer) {
            if isOriginal(word: lowerAnswer) {
                if isWord(word: lowerAnswer) {
                    ...
                    errorMessage = "Enter a real word"
                }
            } else {
                errorTitle = "Already used word"
                errorMessage = "Enter a new word"
            }
        } else {
            errorTitle = "Invalid Word"
            errorMessage = "Enter a word that uses \(title!), is not equal to \(title!), and is longer than two characters"
        }
    ...
    func isPossible(word: String) -> Bool {
        ...
        if word == tempWord || word.utf16.count < 3 { return false }
